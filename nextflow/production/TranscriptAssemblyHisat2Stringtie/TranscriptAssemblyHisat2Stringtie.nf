...
process trimmomatic {
	...
	set sample_id, file('*_unpaired_*.fastq.gz')
...
process hisat2_index {
		...
		output:
		set "${genome_fasta.baseName}.hisat2_index", file('*.ht2') into hisat2_indicies
...
process hisat2 {
	...
	set hisat2_basename, file(hisat2_index_files) from hisat2_indicies.collect()
...
process multiqc {
    ...
    multiqc . -f $rtitle $rfilename \\
        -m fastqc -m trimmomatic -m hisat2 -m stringtie2
    """
}
